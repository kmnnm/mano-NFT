...
const apiKey =
  'eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJzdWIiOiJkaWQ6ZXRocjoweGI4ZEY3ZDU5ODkzOGMwMGRlNzk3NDVFZTVjNkIwZWZlRWI5ZDQ0OEQiLCJpc3MiOiJuZnQtc3RvcmFnZSIsImlhdCI6MTY1MDE5MzM3ODc4OSwibmFtZSI6Im1hbm8tTkZUIn0.t0ryGq4KUwSwrZUzkwK-FI2Ly-pRNpFz_qVM4pEXmh4';
// NFT Storage에서 받은 API 키를 넣는다.
...
const saveImage = (canvas, index) => {
  const filename = `M${index.toString().padStart(3, 0)}`;
  fs.writeFileSync(
...
const uploadMetaData = async (t, i) => {
  ...
  const filename = `M${i.toString().padStart(3, 0)}`;
