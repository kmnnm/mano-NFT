...
function AlsList() {
  ...
  useEffect(() => {
    if (alsNft !== null) {
      ...
      const alsNftInterface = new ethers.utils.Interface(
  ...
  const handleMint = async (e) => {
    if (alsNft !== null) {
      ...
      const tx = await alsNft.mint(OWNER, tokenId, tokenURI, {
        gasLimit: 3000000,
      });
      try {
        await tx.wait();
